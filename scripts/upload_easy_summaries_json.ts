/**
 * data/easy_summaries.json 의 "이해하기 쉬운 ver" 요약을 DB(patents.easy_summary_md)로 적재.
 *
 * 키 규약: "{country}_{doc_id}" 예) "CN_121871527"
 *   → patents.country = 'CN' AND patents.publication_no = '121871527' 매칭.
 *   (upload_summaries_json.ts 와 동일한 매핑 로직)
 *
 * 실행:
 *   cd web && NODE_PATH=./node_modules ./node_modules/.bin/tsx ../scripts/upload_easy_summaries_json.ts
 *   cd web && NODE_PATH=./node_modules ./node_modules/.bin/tsx ../scripts/upload_easy_summaries_json.ts --dry-run
 *   cd web && NODE_PATH=./node_modules ./node_modules/.bin/tsx ../scripts/upload_easy_summaries_json.ts --limit 10
 */
...
const SRC = path.join(ROOT, "data/easy_summaries.json");
const SCRAPED = path.join(ROOT, "data/descriptions_scraped.json");
...
async function main() {
  ...
  // Fallback: easy_key → skey from descriptions_scraped.json
  let scrapedSkey: Record<string, { skey?: string }> = {};
  if (fs.existsSync(SCRAPED)) {
    scrapedSkey = JSON.parse(fs.readFileSync(SCRAPED, "utf-8"));
  }

  function resolveWipsonKey(jsonKey: string): string | null {
    const direct = idx.get(jsonKey.toUpperCase());
    if (direct) return direct;
    const skey = scrapedSkey[jsonKey]?.skey;
    if (skey && wipsonSet.has(skey)) return skey;
    return null;
  }

  const data = JSON.parse(fs.readFileSync(SRC, "utf-8")) as Record<string, EasyRec>;
  ...
  for (const u of toApply) {
    ...
    try {
      await pool.query(
        `update patents set easy_summary_md = $1 where wipson_key = $2`,
        [u.text, u.wipsonKey],
      );
      ok++;
