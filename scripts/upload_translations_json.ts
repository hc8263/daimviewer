/**
 * data/descriptions_translated.json 의 한글 번역(text_kr)을 DB(patents.description_ko)로 적재.
 *
 * 키 규약: "{country}_{doc_id}" 예) "CN_121871527"
 *   → patents.country = 'CN' AND patents.publication_no = '121871527' 매칭.
 *
 * 실행:
 *   cd web && NODE_PATH=./node_modules ./node_modules/.bin/tsx ../scripts/upload_translations_json.ts
 *   cd web && NODE_PATH=./node_modules ./node_modules/.bin/tsx ../scripts/upload_translations_json.ts --dry-run
 *   cd web && NODE_PATH=./node_modules ./node_modules/.bin/tsx ../scripts/upload_translations_json.ts --limit 10
 *
...
const SRC = path.join(ROOT, "data/descriptions_translated.json");
...
async function main() {
  ...
  // 매핑 인덱스
  //  - 기본: "{CTRY}_{publication_no}" → wipson_key
  //  - fallback: doc_id 가 비어있는 경우 키 suffix 가 wipson_key 자체 → wipson_key set
  ...
  function resolveWipsonKey(jsonKey: string, rec: TranslatedRec): string | null {
    const direct = idx.get(jsonKey.toUpperCase());
    if (direct) return direct;
    // fallback: doc_id 비어있어 키가 "{ctry}__{wipson_key}" 형태
    if (!rec.doc_id) {
  ...
  const data = JSON.parse(fs.readFileSync(SRC, "utf-8")) as Record<string, TranslatedRec>;
  const entries = Object.entries(data);

  const updates: { wipsonKey: string; key: string; text: string; bytes: number }[] = [];
  let missingDb = 0;
  let missingText = 0;
  for (const [key, rec] of entries) {
    const text = rec?.text_kr;
    if (!text || !text.trim()) { missingText++; continue; }
    const wkey = resolveWipsonKey(key, rec);
    if (!wkey) { missingDb++; continue; }
    updates.push({ wipsonKey: wkey, key, text, bytes: text.length });
  }
  ...
  console.log(`json w/o text_kr:    ${missingText}`);
  ...
  for (const u of toApply) {
    ...
    try {
      await pool.query(
        `update patents set description_ko = $1 where wipson_key = $2`,
        [u.text, u.wipsonKey],
      );
      ok++;
